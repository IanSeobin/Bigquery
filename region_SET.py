...
SELECT
A.country, A.region, A.fromTIME, A.toTIME, AU, SESSION_AVG_DUR
FROM
    (
    select
      country, region
    ...
    (
    select
    country, region, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from
        (
        select
          country, region, SESSION_ID_ALL
        , MIN(CONCAT(REPLACE(FORMAT_DATETIME('%F', timestamp),'-', ''),'0000')) fromTIME
        , MAX(CONCAT(REPLACE(FORMAT_DATETIME('%F', timestamp),'-', ''),'2359')) toTIME
     ...
     group by 1,2
    ) B ON A.fromTIME = B.fromTIME AND A.toTIME = B.toTIME AND A.country=B.country AND A.region=B.region
""".format(int(sys.argv[1]), int(sys.argv[2]))

query = client_BQ.query(sql)
data = query.to_dataframe()
data.to_sql('region_SET', con=db_connection, if_exists='replace', index=False)
