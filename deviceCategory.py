...
SELECT
A.category, A.fromTIME, A.toTIME, AU, SESSION_AVG_DUR
FROM 
    (
    select
      category
    ...
    (  
    select
    category, time, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
          category
    ...
    ) B ON A.fromTIME=B.fromTIME AND A.toTIME=B.toTIME AND A.category=B.category
""".format(int(sys.argv[1]), int(sys.argv[2]), str(sys.argv[3]))

query = client_BQ.query(sql)
data = query.to_dataframe()
data.to_sql('deviceCategory', con=db_connection, if_exists='replace', index=False)
