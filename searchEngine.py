...
FROM 
    ...
    (  
    select
    source, time, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
          source
...
data.to_sql('searchEngine', con=db_connection, if_exists='replace', index=False)
