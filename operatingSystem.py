...
SELECT
A.operating_system, A.fromTIME, A.toTIME, AU, SESSION_AVG_DUR
FROM 
    (
    select
    operating_system
    ...
    (  
    select
    operating_system, time, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
          operating_system
    ...
    group by operating_system, time
    ) B ON A.fromTIME=B.fromTIME AND A.toTIME=B.toTIME AND A.operating_system=B.operating_system
""".format(int(sys.argv[1]), int(sys.argv[2]), str(sys.argv[3]))

query = client_BQ.query(sql)
data = query.to_dataframe()
data.to_sql('operatingSystem', con=db_connection, if_exists='replace', index=False)
