...
FROM 
    ...
    (  
    select
    country, region, time, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
          country, region
...
data.to_sql('region', con=db_connection, if_exists='replace', index=False)
