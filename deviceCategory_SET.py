...
FROM 
    ...
    (  
    select
    category, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
          category
...
data.to_sql('deviceCategory_SET', con=db_connection, if_exists='replace', index=False)
