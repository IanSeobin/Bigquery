...
where _TABLE_SUFFIX between '20240501' and '20240502'
and traffic_source.medium in ('organic', 'brandsearch')
)
SELECT
A.source, A.fromTIME, A.toTIME, AU, SESSION_AVG_DUR
FROM 
    (
    select
    source
    ...
    (  
    select
    source, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
          source, SESSION_ID_ALL
        , MIN(CONCAT(REPLACE(FORMAT_DATETIME('%F', timestamp),'-', ''),'0000')) fromTIME
        , MAX(CONCAT(REPLACE(FORMAT_DATETIME('%F', timestamp),'-', ''),'2359')) toTIME
    ...
    group by source
    ) B ON A.fromTIME=B.fromTIME AND A.toTIME=B.toTIME AND A.source=B.source
""".format(int(sys.argv[1]), int(sys.argv[2]))

query = client_BQ.query(sql)
data = query.to_dataframe()
data.to_sql('searchEngine_SET', con=db_connection, if_exists='replace', index=False)
