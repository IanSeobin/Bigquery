...
SELECT
A.newVsReturning, A.fromTIME, A.toTIME, AU, SESSION_AVG_DUR
FROM 
    (
    select
    newVsReturning
    ...
    (  
    select
    newVsReturning, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
          newVsReturning, SESSION_ID_ALL
        , MIN(CONCAT(REPLACE(FORMAT_DATETIME('%F%H', timestamp),'-', ''),'00')) fromTIME
        , MAX(CONCAT(REPLACE(FORMAT_DATETIME('%F%H', timestamp),'-', ''),'59')) toTIME
    ...
    ) B ON A.fromTIME=B.fromTIME AND A.toTIME=B.toTIME AND A.newVsReturning=B.newVsReturning
""".format(int(sys.argv[1]), int(sys.argv[2]))

query = client_BQ.query(sql)
data = query.to_dataframe()
data.to_sql('newVSreturn_SET', con=db_connection, if_exists='replace', index=False)
