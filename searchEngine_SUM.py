...
select
source, fromTIME, toTIME, PAGE_VIEW, SESSIONS, ENGAGED_SESSIONS
from
  (
  select
    REPLACE(FORMAT_DATETIME('%F', timestamp),'-', '')
  , source
...
data.to_sql('searchEngine_SUM', con=db_connection, if_exists='replace', index=False)
