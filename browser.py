...
FROM 
    ...
    (  
    select
    browser, time, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
          browser
...
data.to_sql('browser', con=db_connection, if_exists='replace', index=False)