...
FROM 
    ...
    (  
    select
    time, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
...
data.to_sql('visitor', con=db_connection, if_exists='replace', index=False)
