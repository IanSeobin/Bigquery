# 데이터 내역: 추출자가 원하는 금일 시간(분) 간격으로 업데이트 되는 세션수

# sys.argv[1]: 추출자가 원하는 시간(분) 입력
# ex) 30분: python REAL_session.py 30
# ex) 2시간: python REAL_session.py 120
...
sql = """ 
WITH REAL_visitSession AS (
select
dense_rank() over(order by time desc) rank, 
REPLACE(FORMAT_DATETIME('%F%H%M',time),'-','') fromTIME, 
REPLACE(FORMAT_DATETIME('%F%H%M',timestamp_add(time, interval {0}*60 second)),'-','') toTIME, SESSIONS
from 
  (
  SELECT  
...
select
fromTIME, toTIME, SESSIONS
from REAL_visitSession
where rank <> 1
...
data.to_sql('REAL_session', con=db_connection, if_exists='replace', index=False)