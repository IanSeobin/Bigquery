...
SELECT
A.newVSreturn, A.fromTIME, A.toTIME, AU, SESSION_AVG_DUR
FROM 
    (
    select
      newVSreturn
    ...
    (  
    select
    newVSreturn, time, min(fromTIME) fromTIME, max(toTIME) toTIME, sum(session_second)/count(distinct SESSION_ID_ALL) SESSION_AVG_DUR
    from 
        (
        select
          newVSreturn
    ...
    group by newVSreturn, time
    ) B ON A.fromTIME=B.fromTIME AND A.toTIME=B.toTIME AND A.newVSreturn=B.newVSreturn
""".format(int(sys.argv[1]), int(sys.argv[2]), str(sys.argv[3]))

query = client_BQ.query(sql)
data = query.to_dataframe()
data.to_sql('newVSreturn', con=db_connection, if_exists='replace', index=False)