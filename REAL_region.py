# 데이터 내역: 추출자가 원하는 금일 시간(분) 간격으로 업데이트 되는 지역별 방문자 수

# sys.argv[1]: 추출자가 원하는 시간(분) 입력
# ex) 30분: python REAL_region.py 30
# ex) 2시간: python REAL_region.py 120
...
REPLACE(FORMAT_DATETIME('%F%H%M',timestamp_add(time, interval {0}*60 second)),'-','') toTIME, region, AU
FROM
  (
  SELECT  
...
data.to_sql('REAL_region', con=db_connection, if_exists='replace', index=False)